...
.block-container { 
    background: var(--card) !important; border-radius:14px; 
    padding:24px 22px 18px !important; overflow:visible !important; color: var(--ink) !important;
}
div[data-testid="stHeader"] { height:auto !important; }
div[data-testid="stMarkdownContainer"] h1, h1, h2, h3, h4, h5, h6 { 
    line-height:1.3 !important; margin-top:0.25rem !important; color: var(--ink) !important;
}
[role="radiogroup"] label p { line-height:1.25 !important; color: var(--ink) !important; }
.stButton>button { 
    background: linear-gradient(90deg,var(--accent),var(--accent2));
    color: var(--ink); border:none; border-radius:10px; padding:10px 16px; font-weight:700; 
}
.stButton>button:hover { opacity:.93 }
hr { border-color: var(--accent); }
.small { font-size: 0.9rem; color: var(--muted); }
.card { 
    background: var(--card); padding:14px 14px; border-radius:12px; 
...
.gauge > span { position:absolute; left:0; top:0; bottom:0; width:0%; background:linear-gradient(90deg,var(--accent),var(--accent2));
...
def pretty_targets_text(targets: Dict[str, int]) -> str:
    return "\n".join([f"- {r}: {n}줄" for r, n in targets.items()])

def fallback_hard_adjust(script: str, roles: List[str], targets: Dict[str, int]) -> str:
    """
    최후 수단: 초과분은 지문으로 전환(카운트 제외), 부족분은 긴 대사를 분할해 보충.
    자연스러움 최우선은 아니지만 '개수'는 강제 일치.
    """
    ...
    for r in roles:
        while cur[r] < targets[r]:
            ...
            if len(sentences) >= 2:
                first = sentences[0]
                second = " ".join(sentences[1:]).strip() or "그리고…"
                lines[longest_i] = f"{r}: {first}"
                insert_at = longest_i + 1
                lines.insert(insert_at, f"{r}: {second}")
            else:
                insert_at = longest_i + 1
                lines.insert(insert_at, f"{r}: (잠시 생각하며) 음…")
...
def page_role_balancer():
    ...
    if st.button("🔁 재분배하기", key="btn_rebalance", use_container_width=True):
        ...
        system_rules = (
            ...
            "3) 등장인물은 요청에 제공된 목록만 사용. 새 인물 금지.\n"
            "4) 지문(괄호)은 대사 수 계산에서 제외된다.\n"
            "5) 기승전결은 유지·개선하되, 자연스럽게 연결하라."
        )
...
- 개수만 맞추면 안 되고, 내용이 최대한 자연스럽고 일관되게 들리도록 조정한다.
...
"""
        ...
        for attempt in range(1, MAX_TRIES + 1):
            ...
            try:
                model_counts = json.loads(m.group(1)) if m else {}
            except Exception:
                model_counts = {}

            # 원본에 없는 역할 제거(지문으로 강등) → 줄수에 영향 없이 내용 유지
            filtered_lines = []
            for line in clean_script_text(body).splitlines():
                ...
                if mm:
                    ...
                    if character in roles:
                        filtered_lines.append(f"{character}: {mm.group(2).strip()}")
                    else:
                        filtered_lines.append(f"({mm.group(2).strip()})")
                else:
                    filtered_lines.append(line)
            body = "\n".join(filtered_lines)

            actual = count_dialogue_lines(body, roles)
            if actual == targets_dict:
                final_script = body
                break

            diff_lines = []
            for r in roles:
                diff_lines.append(f"{r}: 실제 {actual.get(r,0)} vs 목표 {targets_dict[r]}")
            diff_txt = "\n".join(diff_lines)

            msgs.append({"role": "assistant","content": txt})
            msgs.append({"role": "user","content": f"""아래와 같이 줄 수가 불일치합니다. 반드시 고쳐서 다시 출력하세요.
...
"""})
        ...
        if final_script is None:
            loading_placeholder.warning("LLM이 목표 줄 수를 100% 맞추지 못해, 안전 후처리를 적용합니다.")
        ...
        st.success("✅ 재분배 완료! (목표 대사 수 강제 검증 적용)")
        st.rerun()
...
def page_rehearsal_partner():
    ...
    voice_label = st.selectbox("상대역 음성 선택", VOICE_KR_LABELS_SAFE, index=0, key="tts_voice_label_safe",
            ...
            if audio_bytes:
                with st.status("🎧 인식 중...", expanded=True) as s:
                    ...
                    if st.session_state.get("auto_done_token") != (cur_idx, token):
                        ...
                        st.caption("비교 기준(지문 제거)"); st.code(expected_core, language="text")
